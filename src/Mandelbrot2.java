...
public class Mandelbrot2 {
	...
	/*public static class Ap {
		private static final int sh = 100000;
		
		public static float[] square(float[] n) {
			double nd[] = new double[n.length];
			float rf[] = new float[n.length];
			
			for (int i=0;i<n.length;i++) {
				nd[i] = 0;
				for (int j=0;j<i+1;j++) {
					nd[i] += (double)n[j] * (double)n[i-j];
				}
			}
			
			double rest = 0;
			for (int i=0;i<n.length;i++) {
				rf[i] = (float)( (int)((nd[i]+rest)*Math.pow(sh, i+1)) /Math.pow(sh, i+1) );
				rest = (nd[i]+rest)*Math.pow(sh, i+1)%1/Math.pow(sh, i+1);
			}
			
			/ *for (int i=n.length-1;i>0;i--) {
				rf[i-1]+=(int)(rf[i]*Math.pow(sh, i));
				rf[i]=(float)(rf[i]*Math.pow(sh, i)%1/Math.pow(sh, i) );
			}* /
	...
	/*public static class Complex {
		...
		public Complex add(Complex _c) {
			return new Complex(r + _c.getReal(), i + _c.getImaginary());
		}
		
		public Complex substract(Complex _c) {
			return new Complex(r - _c.getReal(), i - _c.getImaginary());
		}
		
		public Complex multiply(Complex _c) {
			return new Complex(r*_c.getReal() - i*_c.getImaginary(), r*_c.getImaginary() + i*_c.getReal());
		}
		
		public Complex pow(double _p) {
			double teta = Math.atan(i/r);
			double absv = Math.sqrt(i*i + r*r);
			
			return new Complex(Math.pow(absv, _p)*Math.cos(_p*teta), Math.pow(absv, _p)*Math.sin(_p*teta));
		}
	}*/
	...
	public static class MandelKernel extends Kernel{		
		...
		public void run() {
			...
			while (count < maxIterations && zx*zx + zy*zy < 8) {
				...
				zx=Math.pow(absv, p)*Math.cos(p*teta)+x;
				zy=Math.pow(absv, p)*Math.sin(p*teta)+y;
	...
	public static void listen() {
		frame.addComponentListener(new ComponentListener() {
			public void componentResized(ComponentEvent e) {
				...
				if (resizing==false) {
					resizing=true;
					(new Thread(new Mandelbrot2.resizeThread() ) ).start();
				}								
		    }
		...
		imagepanel.addMouseListener(new MouseListener() {
			public void mousePressed(MouseEvent e) {
				if (e.getButton()==1) {
					...
					dragsy=e.getY();
					dragging=true;
					(new Thread(new Mandelbrot2.dragThread(dragsx, dragsy) ) ).start();
				}
